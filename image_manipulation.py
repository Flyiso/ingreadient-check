...
class ManageFrames:
    ...
    def find_text(self, frame, output_type: str = "dict", lang: str = "swe") -> dict:
        """
        Returns data dictionary of the text
        found in the frame.
        """
        data = pt.image_to_data(
            frame, config=self.config, lang=lang, output_type=output_type
        )
        return data
    ...
    def set_threshold_values(self, frame, frame_data: dict):
        ...
        frame = frame[img_x : img_x + img_w, img_y : img_y + img_h]
        self.target_area = frame
        self.hue_threshold1 = np.min(
            cv2.cvtColor(self.target_area, cv2.COLOR_BGR2HSV), axis=(0, 1)
        ).astype(int)
        self.hue_threshold2 = np.max(
            cv2.cvtColor(self.target_area, cv2.COLOR_BGR2HSV), axis=(0, 1)
        ).astype(int)
        ...
        self.gs_threshold1 = int(
            (
                np.min(
                    cv2.cvtColor(self.target_area, cv2.COLOR_BGR2GRAY), axis=(0, 1)
                ).astype(int)
            )
            * 0.35
        )
        self.gs_threshold2 = int(
            np.max(
                cv2.cvtColor(self.target_area, cv2.COLOR_BGR2GRAY), axis=(0, 1)
            ).astype(int)
            * 1.35
    ...
    def get_roi_mask(self, frame):
        ...
        cl_mask = cv2.inRange(hsv_frame, self.hue_threshold1, self.hue_threshold2)
    ...
    def warp_img(self, frame):
        ...
        ret, thresh1 = cv2.threshold(gs_f, gs_f.mean(), gs_f.max(), cv2.THRESH_BINARY)
        contours, hierarchy = cv2.findContours(
        ...
        if isinstance(approx, np.ndarray) and bool(points_1) is True:
            matrix = cv2.getPerspectiveTransform(
                np.float32(points_1), np.float32(points_2)
            )
    ...
    def _get_lines(self, frame, thresh: int = 50, line_len: int = 30, gap: int = 5):
        """
        ties to find the direction of the text.
        """
        frame_g = cv2.GaussianBlur(frame, (9, 9), 0)
        frame_g = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
        edges = cv2.Canny(frame_g, self.gs_threshold1, self.gs_threshold2, None, 3)
    ...
    def rotate_by_lines(self, frame):
        ...
        rotated_frame = cv2.warpAffine(
            frame, rotation_matrix, (width, height), cv2.INTER_LINEAR
        )
    ...
    def crop_out_contour(self, frame, largest_approximation):
        ...
        cv2.drawContours(
            contour_mask, [largest_approximation], -1, (255), thickness=cv2.FILLED
        )
    ...
    def warp_frame(self, frame, points_a, points_b, shape):
        he = shape[0]
        wi = shape[1]
        matrix = cv2.getPerspectiveTransform(np.float32(points_a), np.float32(points_b))
    ...
    def draw_detect_keypoints(self, frame):
        ...
        frame_gs = cv2.drawKeypoints(
            frame, key_points, frame, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
        )
        return frame_gs
    ...
    def hugh_lines_mask(self, frame):
        ...
        for line in lines:
            ...
            if (x1 - x2) != 0 and abs((y1 - y2) / (x1 - x2)) < 0.15:
                cv2.line(mask, (x1, y1), (x2, y2), (255, 255, 255), 6)
        return mask
